...
async def extract_media(bot: TelegramClient, event: EventDetails):
    ...
    with User() as db:
        if not (
            user := db.fetch(
                columns=["language", "telegram_id"], linkedin_id=event.sender, size=1
            )
        ):
            return
    ...
    if images := event.media.images:
        for image in images:
            error, error_type = await send_media(
                bot, telegram_id, caption, lang, message, image
            )
            media_db.create(
                dnr=dnr_id,
    ...
    if videos := event.media.videos:
        for video in videos:
            error, error_type = await send_media(
                bot, telegram_id, caption, lang, message, video.url
            )
            media_db.create(
                dnr=dnr_id,
                user_tg_id=telegram_id,
    ...
    if documents := event.media.documents:
        for document in documents:
            error, error_type = await send_media(
                bot, telegram_id, caption, lang, message, document
            )
            media_db.create(
                dnr=dnr_id,
                user_tg_id=telegram_id,
...
async def authenticate(bot: TelegramClient, message: str, linkedin_id: str):
    ...
    if code == saved_code:
        with User() as db:
            db.update(
                {"linkedin_id": linkedin_id}, filters={"telegram_id": telegram_id}
            )
...
async def accept_all_invitations():
    ...
    api = Linkedin(settings.LINKEDIN["USER"], settings.LINKEDIN["PASS"])
    redis = Redis()
        ...
        while True:
            ...
            for invite in invites:
                ...
                api.reply_invitation(
                    invite.entity_urn, invite.shared_secret, action="accept"
                )
