...
async def set_language(message: Message):
    ...
    await message.respond(
        f"Please select your preferred language 👇:", buttons=keyboards.SET_LANG
    )
...
async def link_linkedin(message: Message):
    ...
    with User() as db:
        ...
        if (
            not (
                linkedin := db.fetch(
                    columns="linkedin_id", telegram_id=sender.id, size=1
                )
            )
            or not linkedin[0]
        ):
            ...
            text = (
                texts.LinkYourLinkedIn()
                .get(lang)
                .replace("%code%", f"{sender.id}:{code}")
            )

            await message.respond(text, buttons=keyboards.OpenMyLinkedin().get(lang))
            raise events.StopPropagation
